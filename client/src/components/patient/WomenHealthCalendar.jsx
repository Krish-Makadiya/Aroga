...
const DAILY_LOG_SYMPTOMS = [
    "cramps", "bloating", "headache", "fatigue", "acne", "nausea", "moodSwing", "backPain", "heavyBleeding", "spotting", "clotting"
];
...
const WomenHealthCalendar = ({ patientId }) => {
    ...
    const handleToggleSymptom = (symptom) => {
        setLogSymptoms((prev) => prev.includes(symptom) ? prev.filter(s => s !== symptom) : [...prev, symptom]);
    };
    const handleSaveLog = async () => {
        setLogLoading(true);
        try {
            await axios.post(`${import.meta.env.VITE_SERVER_URL}/api/womenhealth/daily-log`, {
                userId: patientId,
                date: selectedDate,
    ...
    const fetchPeriodData = async () => {
        try {
            const response = await axios.get(`${import.meta.env.VITE_SERVER_URL}/api/womenhealth/${patientId}`);
            if (response.data && response.data.dailyLogs) {
                setDailyLogs(response.data.dailyLogs);
            } else {
                setDailyLogs([]);
            }
            if (response.data && response.data.cycles && response.data.cycles.length) {
                // Gather all periodDays, cycleStarts, and cycleEnds from all cycles
                const allPeriodDays = [];
                const allCycleStarts = [];
                const allCycleEnds = [];
                response.data.cycles.forEach(cycle => {
                    if (cycle.periodDays) allPeriodDays.push(...cycle.periodDays.map(d => new Date(d)));
                    if (cycle.cycleStart) allCycleStarts.push(new Date(cycle.cycleStart));
                    if (cycle.cycleEnd) allCycleEnds.push(new Date(cycle.cycleEnd));
                });
            ...
            let ovulationDays = [], fertileWindows = [];
            if (response.data.cycles && response.data.cycles.length) {
                response.data.cycles.forEach(cycle => {
                    if (cycle.ovulationDate) ovulationDays.push(new Date(cycle.ovulationDate));
                    if (cycle.fertileWindow && cycle.fertileWindow.start && cycle.fertileWindow.end) {
                        fertileWindows.push({
            ...
            setSummary({
                nextPeriodStart: response.data.nextPeriodStart || (response.data.cyclePredictions && response.data.cyclePredictions.nextPeriodStart),
    ...
    const isPeriodDay = (day) => {
        ...
        return periodDays.some(d => d.toDateString() === dateObj.toDateString());
    };
    const isPeriodStart = (day) => {
        if (!cycleStart || cycleStart.length === 0) return false;
        const dateObj = new Date(currentMonth.getFullYear(), currentMonth.getMonth(), day);
        return cycleStart.some(start => start.toDateString() === dateObj.toDateString());
    };
    const isPeriodEnd = (day) => {
        if (!cycleEnd || cycleEnd.length === 0) return false;
        const dateObj = new Date(currentMonth.getFullYear(), currentMonth.getMonth(), day);
        return cycleEnd.some(end => end.toDateString() === dateObj.toDateString());
    };
    ...
    const handlePeriodAction = async (action, dateObj) => {
        ...
        if (!dateToUse) return;
        try {
            await axios.post(`${import.meta.env.VITE_SERVER_URL}/api/womenhealth/period-day`, {
                userId: patientId,
                date: dateToUse,
                action,
            });
            ...
            <div className="dark:bg-dark-bg bg-light-surface rounded-2xl p-6 shadow-md hover:shadow-xl transition-all duration-300">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex gap-2">
                        ...
                        {(() => {
                            ...
                            let periodStarted = false, periodEnded = false;
                            if (cycleStart && Array.isArray(cycleStart)) {
                                periodStarted = cycleStart.some(start => start.toDateString() === selectedDate?.toDateString());
                            }
                            if (cycleEnd && Array.isArray(cycleEnd)) {
                                periodEnded = cycleEnd.some(end => end.toDateString() === selectedDate?.toDateString());
                            }
                            // Find if current date is in an active period (started but not ended)
                            let inActivePeriod = false;
                            if (cycleStart && Array.isArray(cycleStart) && cycleEnd && Array.isArray(cycleEnd)) {
                            ...
                            if (cycleStart && Array.isArray(cycleStart) && cycleEnd && Array.isArray(cycleEnd) && selectedDate) {
                                // Sort cycles by start date
                                const cycles = cycleStart.map((start, i) => ({ start, end: cycleEnd[i] || null }));
                                cycles.sort((a, b) => a.start - b.start);
                                for (let i = 0; i < cycles.length - 1; i++) {
                                    const end = cycles[i].end;
                                    const nextStart = cycles[i + 1].start;
                                    if (end && selectedDate > end && selectedDate < nextStart) {
                                        betweenCycles = true;
                                        break;
                                    }
                                }
                                ...
                                for (let i = 0; i < cycles.length; i++) {
                                    ...
                                    const end = cycles[i].end;
                                    if (start < selectedDate && end && selectedDate <= end) {
                                        betweenCycles = true;
                                        break;
                                    }
                                }
                ...
                {showLogModal && (
                    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/60">
                        <div className="bg-white dark:bg-[#0f172a] text-gray-900 dark:text-gray-100 rounded-2xl shadow-xl p-6 w-full max-w-md relative border border-gray-200 dark:border-gray-800">
                            <button
                            ...
                            <h3 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">
                                Add Daily Log ({selectedDate ? new Date(selectedDate).toLocaleDateString() : ''})
                            </h3>

                            <div className="mb-4">
                                <div className="font-semibold mb-2 text-gray-800 dark:text-gray-300">Symptoms</div>
                ...
                ${logSymptoms.includes(s)
                            ...
                            <div className="mb-4">
                                <div className="font-semibold mb-2 text-gray-800 dark:text-gray-300">Flow Intensity</div>
                            ...
                            <div className="mb-4">
                                <div className="font-semibold mb-1 text-gray-800 dark:text-gray-300">Mood (optional)</div>
                                <input
                                    type="text"
                                    value={logMood}
                                    onChange={e => setLogMood(e.target.value)}
                                    className="w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-[#020617] text-gray-900 dark:text-gray-100 px-3 py-2 text-sm mt-1 focus:ring-2 focus:ring-green-500 outline-none"
                                    placeholder="How do you feel?"
                                />
                            </div>

                            <div className="mb-5">
                                <div className="font-semibold mb-1 text-gray-800 dark:text-gray-300">Notes (optional)</div>
                                <textarea
                                    value={logNotes}
                ...
                {showViewLogModal && viewLog && (
                    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/60">
                        <div className="bg-white dark:bg-[#0f172a] text-gray-900 dark:text-gray-100 rounded-2xl shadow-xl p-6 w-full max-w-md relative border border-gray-200 dark:border-gray-800">
                            ...
                            <h3 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">
                                Log Details {viewLog.date ? `(${new Date(viewLog.date).toLocaleDateString()})` : ''}
                            </h3>
                            ...
                            <div className="mb-3">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {Array.isArray(viewLog.symptoms) && viewLog.symptoms.length ? (
                                        viewLog.symptoms.map((symptom, index) => (
                                            <span
                                                key={index}
                            ...
                            <div className="mb-2">
                                <span className="font-semibold text-gray-700 dark:text-gray-300">Flow Intensity:</span>
                            ...
                            <div className="mb-2">
                                <span className="font-semibold text-gray-700 dark:text-gray-300">Mood:</span>
                            ...
                            <div className="flex justify-end mt-5">
                                <button
                                    onClick={() => setShowViewLogModal(false)}
                                    className="px-5 py-2 rounded-lg bg-green-600 hover:bg-green-700 text-white font-medium shadow-lg shadow-green-600/20 transition"
                                >
                                    Close
                                </button>
                            </div>
                            ...
                            <div className="mb-6 space-y-3">
                                <div>
                                    ...
                                    <span className="text-blue-500 dark:text-blue-400 font-bold">
                                        {summary.nextPeriodStart ? " " + new Date(summary.nextPeriodStart).toLocaleDateString() : '--'}
                                    </span>
                                </div>
                                ...
                                {(() => {
                                    if (summary.ovulationDays && summary.ovulationDays.length > 0) {
                                ...
                                {(() => {
                                    if (summary.fertileWindows && summary.fertileWindows.length > 0) {
                                        const lastWin = summary.fertileWindows[summary.fertileWindows.length - 1];
                                        return (
                                            <>
                                                <div>
                            ...
                            <div className="mt-4 text-center">
                                ...
                                <div className="mt-2 text-base">
                                    {(() => {
                                        ...
                                        const selected = new Date(dateToShow.getFullYear(), dateToShow.getMonth(), dateToShow.getDate());
                                        if (summary.ovulationDays && Array.isArray(summary.ovulationDays)) {
                                            for (const ovu of summary.ovulationDays) {
                                                if (selected.toDateString() === ovu.toDateString()) {
                                                    return <span className="text-purple-600 dark:text-purple-400 font-bold">Ovulation Day</span>;
                                                }
                                            }
                                        }
                                        if (summary.fertileWindows && Array.isArray(summary.fertileWindows)) {
                                            for (const win of summary.fertileWindows) {
                                                if (selected >= win.start && selected <= win.end) {
                                                    return <span className="text-green-600 dark:text-green-400 font-bold">Fertility Phase</span>;
                                                }
                                            }
                                        }
                                        if (isPeriodStart(day))
                                            return <span className="text-blue-500 dark:text-blue-400 font-bold">Period Start</span>;

                                        if (isPeriodEnd(day))
                                            return <span className="text-red-500 dark:text-red-400 font-bold">Period End</span>;

                                        if (isPeriodDay(day))
                                            return <span className="text-pink-500 dark:text-pink-400 font-bold">Period Day</span>;
                        ...
                        `}>
                                    {day && (
                                        <>
                                            ...
                                            {isPeriodDay(day) && (
                                                <div className="absolute top-1 right-1">
                                                    <div className={`w-2 h-2 rounded-full 
                                                        ${isPeriodStart(day) ? 'bg-blue-500' : isPeriodEnd(day) ? 'bg-red-500' : 'bg-pink-400'}
                                                    `}></div>
                                                </div>
                                            )}
                                            {/* Fertility phase green dot for all cycles */}
                                            {(() => {
                                                if (!day || !summary.fertileWindows || !Array.isArray(summary.fertileWindows)) return null;
                                                const dateObj = new Date(currentMonth.getFullYear(), currentMonth.getMonth(), day);
                                                for (const win of summary.fertileWindows) {
                                                    if (dateObj >= win.start && dateObj <= win.end && !isPeriodDay(day)) {
                                                        return (
                                                            <div className="absolute top-1 right-1">
                                            ...
                                            {(() => {
                                                if (!day || !summary.ovulationDays || !Array.isArray(summary.ovulationDays)) return null;
                                                const dateObj = new Date(currentMonth.getFullYear(), currentMonth.getMonth(), day);
                                                for (const ovu of summary.ovulationDays) {
                                                    if (dateObj.toDateString() === ovu.toDateString()) {
                                                        return (
                                            ...
                                            {(() => {
                                                if (!day || !summary.nextPeriodStart) return null;
                                                const dateObj = new Date(currentMonth.getFullYear(), currentMonth.getMonth(), day);
                                                if (dateObj.toDateString() === new Date(summary.nextPeriodStart).toDateString()) {
                                                    return (
