...
const WomenHealthHistory = ({ patientId }) => {
    ...
    const handleAiReview = async () => {
        setAiLoading(true); setAiError(""); setAiInsight(null);
        try {
            const res = await axios.get(`${import.meta.env.VITE_SERVER_URL}/api/womenhealth/${patientId}`);
            if (res.data) {
                // Filter cycles/logs for selected range
                const startDate = new Date(aiStartYear, aiStartMonth, 1);
                const endDate = new Date(aiEndYear, aiEndMonth + 1, 0, 23, 59, 59, 999);
                const filteredCycles = (res.data.cycles || []).filter(cycle => {
                    const s = new Date(cycle.cycleStart);
                    const e = cycle.cycleEnd ? new Date(cycle.cycleEnd) : null;
                    return (s <= endDate && (!e || e >= startDate));
                });
                const filteredLogs = (res.data.dailyLogs || []).filter(log => {
                ...
                const aiRes = await axios.post(`${import.meta.env.VITE_SERVER_URL}/api/womenhealthai/review`, { prompt: aiPrompt });
    ...
    const handleAlertClick = async () => {
        ...
        try {
            ...
            const data = res.data;
            const alertsList = [];
            if (data && data.cyclePredictions && data.cyclePredictions.nextPeriodStart) {
                const predictedDate = new Date(data.cyclePredictions.nextPeriodStart);
                const today = new Date();
                // Find if user has marked period start after predicted date
                let hasMarkedStart = false;
                if (Array.isArray(data.cycles)) {
                    for (const cycle of data.cycles) {
                        if (cycle.cycleStart && new Date(cycle.cycleStart) > predictedDate) {
                            hasMarkedStart = true;
                            break;
                        }
                    }
                ...
                const diffDays = Math.floor((predictedDate - today) / (1000 * 60 * 60 * 24));
    ...
    return (
        <>
            <div className="w-full flex justify-center mt-2 mb-2">
                <div className="flex gap-4 dark:bg-dark-bg bg-light-surface rounded-xl px-6 py-2 shadow">
                    <button className="px-4 py-2 rounded-lg bg-green-600 hover:bg-green-700 text-white font-semibold transition" onClick={() => setShowCycleModal(true)}>
                        Cycle History
                    </button>
                    <button className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold transition" onClick={() => setShowAiModal(true)}>
                        AI Review
                    </button>
                    <button className="relative px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white font-semibold transition" onClick={handleAlertClick}>
                        Alerts
                        {alertCount > 0 && (
                            <span className="absolute -top-2 -right-2 bg-yellow-400 text-black rounded-full px-2 text-xs font-bold">{alertCount}</span>
                        )}
                    </button>
                </div>
            </div>
            ...
            {showAlertModal && (
                <div className="fixed inset-0 z-50 flex flex-col items-center justify-start bg-black/70 overflow-y-auto min-h-screen w-full">
                    <div className="w-full max-w-md bg-white dark:bg-dark-bg rounded-2xl shadow-xl p-6 mt-12 mb-8 relative flex flex-col">
                        <button className="absolute top-3 right-3 text-gray-400 hover:text-gray-700" onClick={() => setShowAlertModal(false)}>
                            ✕
                        </button>
                        <h3 className="text-xl font-bold mb-4 text-center">Health Alerts</h3>
                        {alerts.length === 0 && <div className="text-gray-500 text-center">No alerts at this time.</div>}
                        <ul className="space-y-4">
                            {alerts.map((alert, i) => (
                                <li key={i} className={`rounded-xl p-4 shadow border-l-4 ${alert.type === 'delayed' ? 'border-red-500 bg-red-50 dark:bg-red-900/20' : 'border-yellow-400 bg-yellow-50 dark:bg-yellow-900/20'}`}>
                                    <div className="font-semibold mb-1">
                                        {alert.type === 'delayed' ? '⚠️ Delayed Period' : '🌸 Upcoming Period'}
                                    </div>
                    ...
                    <div className="w-full max-w-2xl bg-white dark:bg-gray-900 rounded-none md:rounded-2xl shadow-2xl p-6 mt-8 mb-8 relative flex flex-col border border-gray-200 dark:border-gray-700">
                        ...
                        <div className="flex flex-wrap gap-4 mb-6 justify-center">
                            ...
                            <div>
                                ...
                                <select
                                    value={aiEndMonth}
                                    onChange={e => setAiEndMonth(Number(e.target.value))}
                                    className="rounded px-2 py-1 mr-2 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600"
                                >
                                    {months.map((m, i) => (
                                        <option value={i} key={m}>{m}</option>
                                ...
                                <select
                                    value={aiEndYear}
                                    onChange={e => setAiEndYear(Number(e.target.value))}
                                    className="rounded px-2 py-1 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600"
                                >
                                    {years.map(y => (
                                        <option value={y} key={y}>{y}</option>
                            ...
                            <button
                                ...
                                className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold ml-2 transition"
                            >
                                Get AI Review
                            </button>
                        </div>
                                ...
                                <div className="space-y-6 mt-4">
                                    ...
                                    <div className="bg-purple-50 dark:bg-purple-900/30 rounded-2xl p-5 shadow border border-purple-100 dark:border-purple-800">
                                        ...
                                        <div className="mb-1 text-gray-900 dark:text-gray-100">
                                            <span className="font-semibold">Next Period Date:</span> {aiInsight.fertilitySummary?.nextPeriodDate || "--"}
                                        </div>
                                        <div className="mb-1 text-gray-900 dark:text-gray-100">
                                            <span className="font-semibold">Ovulation Date:</span> {aiInsight.fertilitySummary?.ovulationDate || "--"}
                                        </div>
                                        <div className="mb-1 text-gray-900 dark:text-gray-100">
                                            <span className="font-semibold">Fertile Window:</span> {aiInsight.fertilitySummary?.fertileWindow || "--"}
                                        </div>
                                    </div>
                                    ...
                                    {aiInsight.riskInsights?.length > 0 && (
                                        <div className="bg-amber-50 dark:bg-amber-900/30 rounded-2xl p-5 shadow border border-amber-100 dark:border-amber-800">
                                            ...
                                            <ul className="list-disc pl-6 text-gray-900 dark:text-gray-100">
                                                {aiInsight.riskInsights.map((r, i) => (
                                                    <li key={i}>{r}</li>
                                                ))}
                                            </ul>
                                        </div>
                                    ...
                                    <div className="bg-emerald-50 dark:bg-emerald-900/30 rounded-2xl p-5 shadow border border-emerald-100 dark:border-emerald-800">
                                        ...
                                        <ul className="list-disc pl-6 text-gray-900 dark:text-gray-100">
                                            {aiInsight.selfCareTips?.map((tip, i) => (
                                                <li key={i}>{tip}</li>
                                            ))}
                                        </ul>
                                    </div>
                                    ...
                                    <div className="bg-red-50 dark:bg-red-900/30 rounded-2xl p-5 shadow border border-red-100 dark:border-red-800">
                                        ...
                                        <ul className="list-disc pl-6 text-gray-900 dark:text-gray-100">
                                            {aiInsight.seeDoctorIf?.map((cond, i) => (
                                                <li key={i}>{cond}</li>
                                            ))}
                                        </ul>
                                    </div>
            ...
            {showCycleModal && (
                <div className="fixed inset-0 z-50 flex flex-col items-center justify-start bg-black/70 overflow-y-auto min-h-screen w-full">
                    <div className="w-full max-w-4xl bg-white dark:bg-gray-900 rounded-none md:rounded-2xl shadow-xl p-6 mt-8 mb-8 relative flex flex-col text-gray-900 dark:text-gray-100">
                        ...
                        <h3 className="text-2xl font-bold mb-6 text-center">Cycle History</h3>
                        ...
                        <div className="flex flex-wrap gap-4 mb-6 justify-center items-end">
                            ...
                            {/* Start Date */}
                            <div>
                                <div className="text-sm font-semibold mb-1">Start Month/Year</div>
                                <select
                                    value={startMonth}
                                    onChange={e => setStartMonth(Number(e.target.value))}
                                    className="rounded-lg px-3 py-2 mr-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700"
                                >
                                ...
                                <select
                                    ...
                                    className="rounded-lg px-3 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700"
                                >
                            ...
                            {/* End Date */}
                            <div>
                                <div className="text-sm font-semibold mb-1">End Month/Year</div>
                                <select
                                    value={endMonth}
                            ...
                            <button
                                ...
                                className="h-[42px] px-6 rounded-lg font-semibold bg-green-600 hover:bg-green-700 dark:bg-green-500 dark:hover:bg-green-600 text-white transition"
                            >
                                Get History
                            </button>
                            ...
                            <button
                                ...
                                className="h-[42px] px-6 rounded-lg font-semibold bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white transition print:hidden"
                            >
                                Print / Download
                            </button>
                        </div>
                        ...
                        {error && (
                            <div className="text-red-500 dark:text-red-400 mb-2 text-center">{error}</div>
                        )}
                            ...
                            <table className="w-full text-left border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                                ...
                                <tbody className="bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300">
                                    {cycles.length === 0 && !loading ? (
                                        <tr>
                                            <td colSpan={6} className="text-center py-6 text-gray-500 dark:text-gray-500">
                                        ...
                                        cycles.map((cycle, idx) => (
                                            ...
                                            >
                                                ...
                                                <td className="p-3">{new Date(cycle.cycleStart).toLocaleDateString()}</td>
                                                <td className="p-3">
                                                    {cycle.cycleEnd
                                                        ? new Date(cycle.cycleEnd).toLocaleDateString()
                                                        : <span className="italic text-gray-400 dark:text-gray-500">Ongoing</span>
                                                    }
                                                </td>
                                                <td className="p-3">
                                                    {cycle.periodLength ? `${cycle.periodLength} days` : '--'}
                                                </td>
                                                <td className="p-3">
                                                    {cycle.ovulationDate
                                                ...
                                                <td className="p-3">
                                                    {cycle.fertileWindow?.start
                                                        ? `${new Date(cycle.fertileWindow.start).toLocaleDateString()} to ${new Date(cycle.fertileWindow.end).toLocaleDateString()}`
                                                        : '--'
                                                    }
                                                </td>
                                            </tr>
