...
exports.addOrUpdatePeriodDay = async (req, res) => {
  try {
    ...
    if (!userId || !date || !action) return res.status(400).json({ error: "Missing fields" });

    let womenHealth = await WomenHealth.findOne({ userId });
    if (!womenHealth) {
      womenHealth = new WomenHealth({ userId, cycles: [], dailyLogs: [] });
    }
    ...
    if (action === 'start' || !cycle || cycle.cycleEnd) {
      ...
      if (cycles.length > 0) {
        ...
        if (prevCycle.cycleStart) {
          cycleLength = Math.round((dateObj - new Date(prevCycle.cycleStart)) / (1000 * 60 * 60 * 24));
        }
      }
      ...
      if (cycles.length > 0) {
        ...
        for (let i = 1; i < cycles.length; i++) {
          if (cycles[i].cycleStart && cycles[i - 1].cycleStart) {
            const len = Math.round((new Date(cycles[i].cycleStart) - new Date(cycles[i - 1].cycleStart)) / (1000 * 60 * 60 * 24));
            sum += len;
            count++;
          }
        }
      ...
      nextPeriodStart = new Date(dateObj.getTime() + averageCycleLength * 24 * 60 * 60 * 1000);
      ...
      if (!womenHealth.cyclePredictions) womenHealth.cyclePredictions = {};
      womenHealth.cyclePredictions.nextPeriodStart = nextPeriodStart;
      cycle = { cycleStart: dateObj, periodDays: [dateObj], predictedNextStart: nextPeriodStart, averageLength: averageCycleLength };
      womenHealth.cycles.push(cycle);
    } else {
      // Add to periodDays if not already present
      if (!cycle.periodDays.some(d => new Date(d).toDateString() === dateObj.toDateString())) {
        cycle.periodDays.push(dateObj);
      }
      // Set cycleEnd if 'end'
      if (action === 'end') {
        if (!cycle.cycleStart) {
          return res.status(400).json({ error: "Cannot set period end without a period start." });
        }
        ...
        cycle.fertileWindow = { start: fertileWindowStart, end: fertileWindowEnd };
        // Calculate averagePeriodLength from all cycles
        const lengths = womenHealth.cycles.map(c => c.periodLength || (c.periodDays ? c.periodDays.length : 0)).filter(l => l > 0);
        if (lengths.length > 0) {
          womenHealth.averagePeriodLength = Math.round(lengths.reduce((a, b) => a + b, 0) / lengths.length);
        }
      }
    }
...
exports.addOrUpdateDailyLog = async (req, res) => {
  try {
    ...
    let log = womenHealth.dailyLogs.find(l => new Date(l.date).toDateString() === new Date(date).toDateString());
    if (log) {
      ...
      womenHealth.dailyLogs.push({ date, symptoms, flowIntensity, mood, notes });
    }
    await womenHealth.save();
    res.json({ success: true, womenHealth });
  } catch (err) {
...
exports.getWomenHealthByUserId = async (req, res) => {
  try {
    ...
    const womenHealth = await WomenHealth.findOne({ userId });
    if (!womenHealth) return res.status(404).json({ error: "No women health data found" });
